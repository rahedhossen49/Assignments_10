...
exports.createWork = async (req, res) => {
    try {
        ...
        res.status(404).json({status: "Fail",data: error.toString()});
    }
};
...
exports.updateWork = async (req, res) => {
    try {
        ...
        let postBody = {title: title,classNote: classNote,description: description};

        let result = await WorksModel.updateOne(query,{ $set: postBody },{ upsert: true });
        res.status(200).json({status: "Success",data: result});
    } catch (error) {
        res.status(404).json({status: "Fail",data: error.toString()});
    }
